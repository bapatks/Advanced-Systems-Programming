            ...
            if(str[0]=='(' && str[23]==')')
            {
                if(str[24]==',' || str[24]=='\0')
                {
                    if(str[5]==',' && str[7]==',')
                    {
                        ...
                        sem_wait(&(temp_ptr->lock));
                        //sem_getvalue(&(temp_ptr->lock), &value);
                        ...
                        {
                            //printf("producer waiting on full buffer of userID = %s\n", cell->userID);
                            sem_post(&(temp_ptr->lock));
                            sem_wait(&(temp_ptr->full));
                            sem_wait(&(temp_ptr->lock));
                        }
