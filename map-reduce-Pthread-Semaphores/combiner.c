...
int main(int argc, char* argv[])
{
	...
	if(argc != 4)
    {
        printf("Input format: ./multithreaded <buffer_size> <number_of_users> <input_file>\n");
        return -1;
    }
    ...
    {
        fclose(check); //will be opened again for reading by mapper
    }
    ...
    */
    ...
    for(int i=0; i<user_num; i++)
    {
        ...
        buff[i].done = 0;
    	//printf("%p\n",&buff[i]);
        sem_init(&(buff[i].lock), 0, 1);
        sem_init(&(buff[i].full), 0, 0);
        sem_init(&(buff[i].empty), 0, 0);
    }
